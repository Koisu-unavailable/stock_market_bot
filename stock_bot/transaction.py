...
class Transaction(ABC):
    def __init__(
        self, price: float, user: User, consumables: list[Consumable], amount: int
    ...
    def _check_if_enough(self):
        self.enough = (
            True if self.user.money >= self.price else False
        )  # Consumables made modify this varirable
...
class BuyBroker(Transaction):
    def __init__(
        self, price: float, user: User, consumables: List[Consumable], broker_id: str # type: ignore  # noqa: F821
    ):  # type: ignore  # noqa: F821
...
class BuyConsumable(Transaction):
    def __init__(
        self,
        price: float,
        user: User,
        consumables: List[Consumable], # type: ignore  # noqa: F821
        consumable_id: str,
    ):  
        super().__init__(price, user, consumables, 1)
    ...
    def _update_user(self):
        self.user.money -= self.price
        try:
            self.user.consumables[self.consumable_id] += 1 * self.amount
        except KeyError:
            self.user.consumables[self.consumable_id] = 1 * self.amount
        add_or_update_user(self.user)
...
class SellStock(
    BuyStock
):  # almost exactley the same functionality just with a few signs flipped or at least the inits the same-ish
